...
// 占い結果をサーバーに保存
export async function saveFortuneReading(reading) {
    const token = localStorage.getItem('authToken');
    if (!token) {
        throw new AuthError(ErrorType.UNAUTHORIZED, 'ログインが必要です');
    }
...
// 占い結果をローカルストレージにも保存
export function saveFortuneReadingLocally(reading) {
    try {
...
// ローカルストレージから占い結果を取得
export function getFortuneReadingsFromLocal() {
    try {
...
// サーバーから占い結果を取得
export async function getFortuneReadings() {
    const token = localStorage.getItem('authToken');
    if (!token) {
        throw new AuthError(ErrorType.UNAUTHORIZED, 'ログインが必要です');
    }
...
// 特定の種類の占い結果のみを取得
export function getReadingsByType(readings, type) {
    return readings.filter(reading => reading.type === type);
}
// 占い結果をPDFとしてエクスポート
export async function exportReadingToPDF(reading) {
    const token = localStorage.getItem('authToken');
    if (!token) {
        throw new AuthError(ErrorType.UNAUTHORIZED, 'ログインが必要です');
    }
...
// 占い結果をJSONとしてエクスポート
export function exportReadingToJSON(reading) {
    try {
...
// 占い結果の削除
export async function deleteFortuneReading(readingId) {
    const token = localStorage.getItem('authToken');
    if (!token) {
        throw new AuthError(ErrorType.UNAUTHORIZED, 'ログインが必要です');
    }
    ...
    try {
        const readings = getFortuneReadingsFromLocal();
        const updatedReadings = readings.filter(reading => 'id' in reading && reading.id !== readingId);

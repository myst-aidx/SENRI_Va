...
export const generateHoroscopeReading = async (input, conversationHistory) => {
    ...
    if (isAIMode()) {
        try {
            const response = await import('../gemini').then(module => module.getGeminiResponse(input, conversationHistory));
            return response;
        }
        catch (error) {
    ...
    // モックデータを使用した応答生成
    const sign = ZODIAC_SIGNS[zodiacSign];
    let reading = '';
    // 基本情報
